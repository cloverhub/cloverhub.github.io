...
var resizePizzas = function(size) { 
  ...
  function changePizzaSizes(size) {
    var pizzaContainer = document.getElementsByClassName("randomPizzaContainer"); // replaced document.querySelectorAll with document.getElementsByClass name for better performance
    var dx = determineDx(pizzaContainer[0], size); // get size difference--moved outside of FOR loop since it doesn't need to repeat
    var newwidth = (pizzaContainer[0].offsetWidth + dx) + 'px'; // set width and move this out of the FOR loop
    
    // update all to proper size. The payload in the FOR loop is minimized
...
var pizzasDiv = document.getElementById("randomPizzas"); // Initialized the pizzasDiv variable outside of the FOR loop to see if page load performance increases
for (var i = 2; i < 100; i++) {
...
// Store last Y position so it is not repeatedly calculated
var lastY = 0;
function onScroll() {
...
document.addEventListener('DOMContentLoaded', function() {
  var screenWidth = screen.availWidth; // get screen width
  var screenHeight = screen.availHeight; // get screen height
  var s = 256;
  var cols = screenWidth / s; // calculate number of columns based on screen width
  var rows = screenHeight / s; // calculate number of rows based on screen height
  var numberPizzas = Math.ceil(cols * rows); // find optimum number of pizzas to use
  // 
